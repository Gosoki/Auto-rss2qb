...
## 十九、R35：『只报不改』的发现，与用户对它点的『知道了』

| 词 | 指什么 | 别读成 |
|---|---|---|
| **发现（finding）** | 三类只报不改的巡检结论：`dup` 同一部番被拆成两条 / `mva` 同 bgm 既是番又是剧场版 / `wrb` 绑定看着不对。判据在 `core/anime` 的三个 `suspect_*` | 不是"错误"——程序判不出对错，只判得出"这看着不对" |
| **身份 `ident`** | 这是**哪一条**发现：`kind:{id}@{出生时刻}(+…)`。变了＝另一条发现，本来就该重新报 | 不是 id：SQLite 的 rowid **会回收**，只按 id 认会让一条全新的发现被上一条的已读静默吃掉 |
| **事实指纹 `fact`** | 读它那一刻的情况，`k=v;k=v` 的人话串。与此刻逐字不等就**复活**，并能逐字段讲出"你读过之后变了什么" | 不是"全部字段"：随采集单调增长的计数（种子数、坏种子条数）**有意不进**——那会让用户每采到一条种子重点一次 |
| **『知道了』** | 收进『已读的提示』。按钮**不叫**"不再提醒"——那是做不到的承诺，事实变了它一定会再出现 | 不是"已解决"：程序判不出用户是修好了还是删了，列表里只写"现在仍然成立/已经不成立了" |
| **复活** | 事实变了，这条发现自己回到仪表盘**和推送**里，并在文案前面说明变了什么 | 不需要"过期时间"，也不需要定时清理 |

**指纹里一律是原始位，不是展示串**：`a_state` 那个三态（追番中/人工拒绝/超期忽略·待确认）
**不是单射** —— `(confirmed=0, rejected=0)` 与 `(0,1)` 都映成"超期忽略/待确认"，而横幅尾巴只读 `rejected`。
拿它当指纹，"那部番从待确认掉成超期忽略"（它下面的集从此一集都不会被下）一个字都不动。

**这个决定要落在三处**：仪表盘横幅、`sweep_alerts` 的推送、以及已读列表本身。只落一处的表现是
"横幅没了、手机每 6 小时照旧响"。而推送那一路还有一个反直觉的要求：**归档动作本身不能触发推送**——
去重键是 id 集合哈希，归档一条会让集合变小、撞出一个从没发过的新 key。所以判据是"有没有**新成员**"，
不是"集合变没变"。
