...
# 范围两侧都要 ≥2 位（真·合集用零填充 01-12），才不把『第二季 - 03』的 "2 - 03" 误判成范围而丢单集；
# 且两侧须被非字母数字包围（用 [A-Za-z0-9] 边界），才不把编码标签 "x264-10bit" 的 "264-10" 误判成范围。
_BATCH_RE = re.compile(
    r"合集|整理|搬运|BD-?RIP|\bBatch\b|(?<![A-Za-z0-9])\d{2,3}\s*[-~〜]\s*\d{2,3}(?![A-Za-z0-9])", re.I)
