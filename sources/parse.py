...
# 【位数上界必须 ≥ 集号抽取的上界】(R27) 抽取侧放到 4 位、这条留在 3 位时，
# 四位连续集合集包两侧都匹配不上 —— 守卫的覆盖面比它要守的东西窄，
# 与上面 _BATCH_RE 的 EP 条踩的是同一个坑（那条已经放宽过一次，这条漏了）。
# 【下界也是 1，不是 2】(R31) 原来两侧写的是 `\d{2,3}`，注释里从没说过为什么要有下限 ——
# 而真正的理由（"两侧都要 ≥2 位，才不把『第二季 - 03』的 2 - 03 误判成范围"）
# 由前面那两条**环视**兜着：左边 `(?<![-A-Za-z0-9])`、右边 `(?![-A-Za-z0-9])`，
# 与位数无关。留着下限的代价是 `- 1-9` 这种右端未补零的区间两侧都匹配不上。
# 真库 2248 条实测：放开下界后判定变化 **0 条**。
# tests/test_parse.py 有一条结构守卫钉住"守卫的位数区间要覆盖抽取的位数区间"。
_BARE_RANGE_RE = re.compile(r"(?<![-A-Za-z0-9])\d{1,4}\s*[-~〜]\s*\d{1,4}(?![-A-Za-z0-9])")
...
# 【这个 3 位必须与集号抽取的 4 位同步】(R27) 集号放到 4 位之后这里仍是 3 位，
# 于是 `番名 - 1156-1170`（四位连续集合集包）里 `-` 被当成合法分隔符 → 抽出"第 1156 集"，
# 而 is_batch 的裸区间那条又要求"抽不出集号"才判合集 —— 两道判据同时失效：
# 一个几十 GB 的合集包以单集入库、被自动下回来，还占死去重键 (番, 1156)，
# 真正的第 1156 集来时被写成 skipped 且 info_hash 不可逆。真库形态实测复现过。
# 【位数只放宽上界，**下界必须留在 1**】(R31) R28 修"四位合集包"时把 `\d{1,3}`
# 整个换成了 `\d{2,4}` —— 上界 3→4 是要修的那件事，**下界 1→2 是顺带改的、没人注意**。
# 于是右端只有一位数的连续集区间（`- 1-9`、`- 1-6`、`- 10-2`）不再被前瞻挡住：
# `-` 重新成了合法的集号收尾锚，`- 1-9` 被抽成**第 1 集**，而 `_BARE_RANGE_RE` 两侧
# 都要求 `\d{2,4}`、`1-9` 又匹配不上 —— 与 R28 那条 P1 逐字同形的"两道判据同时失效"，
# 只是换到了另一个角落。补零写法（`- 01-09`）不受影响，所以真库 2248 条今天 0 条命中：
# 这是一条**潜伏的**回归，不是已经在发生的事故。
# 这条前瞻的语义是"连字符后面跟着一个裸数字 ⇒ 这是区间"，位数从来不该有下限。
_EP_END = r"\s*(?:$|[\[【(（]|[-–—](?!\s*\d{1,4}(?![0-9A-Za-z])))"
...
# 【两条都必须与 _EP_PATTERNS 的对应条**逐字同宽**】(R28)
# 抽取侧第 3 条是 `第\s*([一二三四五六七八九十]+|\d+(?:\.\d+)?)\s*[话話集]`、
# 第 5 条是 `(?:EP|Episode)`，而剥离侧这两条曾经少了 `(?:\.\d+)?`、也只认 `EP`。
# 于是 `第11.5话` / `Episode 07` 这两种写法**集号抽得出来、名字却一个字符都不剥**，
# 违反本行下方注释自己立的规矩。后果不是"名字难看"：alias_key 就是番的身份键，
# 每个小数集号各建一部垃圾番（`某番第11.5话`），正片那部收不到这一集；
# 搜索词同样脏（`_SEARCH_STRIP_PATTERNS` 复用同一份 `_EP_MARK`），
# bgm 搜不到、永久停在『待识别』。`- 08.5` 那种写法不受影响（`_EP_TAIL` 本来就带小数）。
_EP_MARK = (r"第\s*(?:[一二三四五六七八九十]+|\d+(?:\.\d+)?)\s*[话話集]",
            ...
            # 认不出集号 ≠ 名字要留着它。
            r"(?<![A-Za-z0-9])(?:EP|Episode)\s*\.?\s*\d{1,4}(?![\dpPiI])")
