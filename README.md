...
| `db/` | 数据层：`__init__`(SQLite/WAL 引擎 + 会话 + 加列自动迁移) / `models`(数据模型:`Setting`/`SourceGroup`;TV=`Anime`+`AnimeTorrent`+`TitleAlias`;剧场版=`Movie`+`MovieTorrent`) |
...
| `pages/` | NiceGUI 界面：`dashboard`(主页 tab:仪表盘/番剧表/待确认/待识别/已忽略/订阅源) / `movies`(tab:仪表盘/列表/待识别/已忽略/订阅源) / `detail` / `settings` / `sources` / `layout` |
