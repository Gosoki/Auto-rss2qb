...
| `db/` | 数据层：`__init__`(SQLite/WAL 引擎 + 会话 + 加列/改表自动迁移) / `models`(数据模型:`Setting`/`SourceGroup`;TV=`Anime`+`AnimeTorrent`+`AnimeAlias`;剧场版=`Movie`+`MovieTorrent`) |
...
| `pages/` | NiceGUI 界面：`anime`(番剧主页 tab:仪表盘/番剧表/待确认/待识别/已忽略/订阅源) / `anime_detail`(番剧详情) / `movies`(剧场版 tab:仪表盘/列表/待识别/已忽略/订阅源) / `settings` / `sources` / `layout` |
