...
from config import BGM_API, ENRICH_TIMEOUT, MIKAN_BASE, PROXY
...
async def _search_one(client, name, est, release):
    ...
    try:
        r = await client.post(
            f"{BGM_API}/v0/search/subjects", headers=_UA,
...
async def _mikan_bridge(client, info_hash):
    ...
    try:
        ep = await client.get(f"{MIKAN_BASE}/Home/Episode/{info_hash}")
        if ep.status_code != 200:
            return None
        m = _MIKAN_BANGUMI_RE.search(ep.text)
        if not m:
            return None
        bg = await client.get(f"{MIKAN_BASE}/Home/Bangumi/{m.group(1)}")
...
async def resolve(names, release_time=None, episode=None, info_hash=None) -> dict | None:
    ...
    kwargs = {"timeout": ENRICH_TIMEOUT, "follow_redirects": True}
    if PROXY:
        kwargs["proxy"] = PROXY
    try:
        async with httpx.AsyncClient(**kwargs) as client:
            ...
            # ③ 取 bgm 元数据（规范名 + 真实放送日）
            display_name, air_raw = None, None
            if bgm_id is not None:
                try:
                    r = await client.get(f"{BGM_API}/v0/subjects/{bgm_id}", headers=_UA)
                    if r.status_code == 200:
                        data = r.json()
                        display_name = data.get("name_cn") or data.get("name") or None
                        air_raw = data.get("date")
                except (httpx.HTTPError, ValueError):
                    pass

        dt = _parse_date(air_raw)
        if dt is None and bgm_id is None:
            return None
        return {
            "bangumi_id": bgm_id,
