...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    # proxy_headers=False：让 netguard/『本机免 token』看到的恒是【直连对端 IP】。
    # 不设的话 uvicorn 默认 proxy_headers=True，会把 ProxyHeadersMiddleware 包在【最外层】
    # （比 install_netguard 挂的 SubnetGuard 还外），对端落在 forwarded_allow_ips 里时用
    # X-Forwarded-For 改写 scope['client'] —— 于是 netguard 那句"看直连对端、XFF 不采信"在
    # 代码层面并不成立。默认信任集只有 127.0.0.1，实测局域网伪造 XFF 打不穿；但那份安全靠的是
    # uvicorn 的默认值，而 FORWARDED_ALLOW_IPS 这个环境变量（.env 也会被 load_dotenv 读进来）
    # 一旦被设成 '*'，网段白名单与本机免 token 会同时失守。这里显式关掉，把它变成本项目自己的契约。
    # 实测关掉后 WebSocket / 静态资源 / socket.io 全部正常；日后真上反向代理且要拿真实客户端 IP，
    # 正确做法是显式写 forwarded_allow_ips="<代理IP>"，而不是把 proxy_headers 打开。
